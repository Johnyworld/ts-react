...
export const CODE = {
    ...
    FLAG_MINE: -5,
    CLICKED_MINE: -6,
    OPENED: 0 // ALL IS OPENED OVER 0
} as const;


// Context API 는 props를 먼 자식컴포넌트까지 보내줘야 하는 불편함을 해소해준다.
// CreateContext 를 통해서 컴포넌트간 공유할 데이터들을 설정해준다.
...
const reducer = (state = initState, action: ReducerActions): ReducerState => {
    switch (action.type) {
        case START_GAME:
            return {
                ...state,
                ...
                openedCount: 0,
                tableData: plantMine(action.row, action.row, action.mine),
                halted: false,
                timer: 0
            }
        case OPEN_CELL: {
